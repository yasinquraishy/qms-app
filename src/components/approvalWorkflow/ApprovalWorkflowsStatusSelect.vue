...
async function fetchStatuses() {
  try {
    ...
    const mappedData = data.approvalWorkflowStatuses.map((status) => ({
      label: status.name,
      value: status.id,
    }))
    ...
    console.error('Error fetching approval workflow statuses:', error)
    if (props.required) {
      statuses.value = []
    } else {
      statuses.value = [{ label: 'All Statuses', value: null }]
    }
  }
}
